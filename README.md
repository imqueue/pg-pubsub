...
databese side events. It is easy imagine that db can send us messages into, so 
called, structural channels, e. g. 'user:insert', 'company:update' or 
'user_company:delete', where such names are generated by some generic trigger which
handels corresponding database operations and send updates to subscribers 
